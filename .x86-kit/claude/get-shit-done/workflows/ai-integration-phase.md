...
```bash
SELECTOR_MODEL=$(node ".claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-framework-selector --raw)
RESEARCHER_MODEL=$(node ".claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ai-researcher --raw)
DOMAIN_MODEL=$(node ".claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-domain-researcher --raw)
PLANNER_MODEL=$(node ".claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-eval-planner --raw)
```

Check config:
```bash
AI_PHASE_ENABLED=$(node ".claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
