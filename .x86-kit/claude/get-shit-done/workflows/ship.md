...
```bash
REVIEW_CMD=$(node ".claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_command --default "" 2>/dev/null)
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(node ".claude/get-shit-done/bin/gsd-tools.cjs" state load 2>/dev/null | head -20)
   ```
