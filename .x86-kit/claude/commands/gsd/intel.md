...
Intel system is disabled. To activate:
  ...
  node .claude/get-shit-done/bin/gsd-tools.cjs config-set intel.enabled true
...
```bash
node .claude/get-shit-done/bin/gsd-tools.cjs intel status
```
...
```bash
node .claude/get-shit-done/bin/gsd-tools.cjs intel diff
```
